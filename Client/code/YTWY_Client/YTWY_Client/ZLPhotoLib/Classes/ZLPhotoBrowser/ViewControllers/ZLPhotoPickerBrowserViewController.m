...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        scrollView.canLongPress = self.canLongPress;
        // 为了监听单击photoView事件
...
-(void)setPageLabelPage:(NSInteger)page{
    self.pageLabel.text = [NSString stringWithFormat:@"%ld / %ld",
                           (long)(page + 1), (unsigned long)self.photos.count];
    if (self.isPush) {
...
- (void)changeRotationDirection:(NSNotification *)noti{
    ...
    flowLayout.minimumLineSpacing = 0;
    flowLayout.minimumInteritemSpacing = 0;
    flowLayout.itemSize = CGSizeMake(self.view.width + ZLPickerColletionViewPadding, self.view.height);
    flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
